...
func TestBuildCursorPreToolUseResponse_WrapsShellCommand(t *testing.T) {
	...
	response, changed, err := buildCursorPreToolUseResponse(strings.NewReader(input), "/usr/local/bin/fence", nil)
	if err != nil {
		t.Fatalf("buildCursorPreToolUseResponse() error = %v", err)
	}
	if !changed {
		t.Fatal("expected Shell command to be rewritten")
	}
	...
	if decoded.Permission != "allow" {
		t.Fatalf("expected permission allow, got %q", decoded.Permission)
	}
	if !decoded.Continue {
	...
	if got := decoded.UpdatedInput["description"]; got != "Run tests" {
		t.Fatalf("expected description to be preserved, got %#v", got)
	}
	if got := decoded.UpdatedInput["run_in_background"]; got != true {
		t.Fatalf("expected run_in_background to be preserved, got %#v", got)
	}
	if got := decoded.UpdatedInput["timeout"]; got != float64(120000) {
		t.Fatalf("expected timeout to be preserved, got %#v", got)
	}
}

func TestBuildCursorPreToolUseResponse_SkipsPureCD(t *testing.T) {
	input := `{
		"hook_event_name": "preToolUse",
		"tool_name": "Shell",
		"tool_input": {
	...
	_, changed, err := buildCursorPreToolUseResponse(strings.NewReader(input), "/usr/local/bin/fence", nil)
	if err != nil {
		t.Fatalf("buildCursorPreToolUseResponse() error = %v", err)
	}
	if changed {
...
func TestBuildCursorPreToolUseResponse_SkipsAlreadyFencedCommand(t *testing.T) {
	input := `{
		"hook_event_name": "preToolUse",
		"tool_name": "Shell",
		"tool_input": {
...
func TestBuildCursorPreToolUseResponse_IgnoresNonShellEvent(t *testing.T) {
	...
	if changed {
		t.Fatal("expected non-Shell event to be ignored")
	}
}

func TestBuildCursorPreToolUseResponse_InvalidJSON(t *testing.T) {
	_, _, err := buildCursorPreToolUseResponse(strings.NewReader(`{`), "/usr/local/bin/fence", nil)
	if err == nil {
		t.Fatal("expected invalid JSON to return an error")
	}
}

func TestBuildCursorPreToolUseResponse_LeavesCommandUnchangedInsideFence(t *testing.T) {
...
func TestBuildCursorPreToolUseResponse_DeniesBlockedCommandInsideFence(t *testing.T) {
	t.Setenv(fenceSandboxEnvVar, "1")
  ...
  "command": {
	...
	if got := decoded.Permission; got != "deny" {
		t.Fatalf("expected permission deny, got %q", got)
	}
	if !decoded.Continue {
		t.Fatal("expected continue=true in deny response")
	}
	if decoded.UpdatedInput != nil {
		t.Fatalf("expected deny response to omit updated_input, got %#v", decoded.UpdatedInput)
	}
}

func TestBuildCursorPreToolUseResponse_UsesPayloadCWDForOuterDeny(t *testing.T) {
	repoDir := t.TempDir()
...
func TestRunCursorPreToolUse_AcceptsClaudePayload(t *testing.T) {
	t.Setenv(fenceSandboxEnvVar, "")
	...
	if err := runCursorPreToolUse(strings.NewReader(input), &stdout, "/usr/local/bin/fence", nil); err != nil {
		t.Fatalf("runCursorPreToolUse() error = %v", err)
	}
	...
	if got := decoded.HookSpecificOutput.PermissionDecision; got != "allow" {
		t.Fatalf("expected permissionDecision allow, got %q", got)
	}
	wantCommand := sandbox.ShellQuote([]string{"/usr/local/bin/fence", "-c", "npm test"})
